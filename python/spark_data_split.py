...
spark_df = pd.read_csv("./data/SPARK_master_dataset.csv")
spark_df = spark_df.rename(columns={'emotion_1': 'emotion'})

# Assign rare labels (≤3 instances) to train only
emotion_counts = spark_df['emotion'].value_counts()
rare_labels = emotion_counts[emotion_counts < 10].index.tolist()
print(f'Rare labels (train only): {rare_labels}')

df_rare = spark_df[spark_df['emotion'].isin(rare_labels)].copy()
spark_df = spark_df[~spark_df['emotion'].isin(rare_labels)].copy()
...
train_df = pd.concat([train_df, df_rare]).sample(
...
def df_to_json(df, text_column='sentence_en_validated', label_column='emotion'):
...
for lang_name, text_col in datasets.items():
    ...
    # Save JSON files
    train_file = f"./data/spark_{lang_name}_train.json"
    val_file = f"./data/spark_{lang_name}_val.json"
    test_file = f"./data/spark_{lang_name}_test.json"
