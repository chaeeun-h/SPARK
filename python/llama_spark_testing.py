...
ADAPTER_DIR = "./llama_spark_english_finetuned"  # Change to "korean" for Korean model
TEST_FILE = "./spark_english_test.json"  # Change to "korean" for Korean test

# Create logs directory
os.makedirs("./logs/llama_spark_english", exist_ok=True)

# Setup logging to file
log_filename = f"./logs/llama_spark_english/evaluation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
# Load label mappings
with open(f"{ADAPTER_DIR}/label_mappings.pkl", "rb") as f:
...
log_print(f"Loaded adapter from {ADAPTER_DIR}")
...
# =========================
log_print("\n" + "="*50)
log_print("Llama SPARK Model Evaluation Results")
log_print("="*50)
...
results_df.to_csv(f"{ADAPTER_DIR}/test_predictions.csv", index=False)

log_print(f"\nSaved detailed predictions to {ADAPTER_DIR}/test_predictions.csv")
