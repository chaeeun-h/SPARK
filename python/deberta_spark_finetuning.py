...
LANGUAGE   = "english"   # fixed from "korean" in original
...
OUTPUT_DIR = f"./data/deberta_spark_{LANGUAGE}_finetuned"

MAX_LENGTH = 128  # DeBERTa-v3 is stable at 128
...
# Filter empty sentences (DeBERTa stability)
dataset = dataset.filter(
    lambda x: x["sentence"] is not None and
              len(str(x["sentence"]).strip()) > 0
)
...
# =========================
# Training args
# (fp16=False and adam_epsilon=1e-6 for DeBERTa-v3 stability)
# =========================
training_args = TrainingArguments(
    output_dir                  = OUTPUT_DIR,
    logging_dir                 = f"./logs/deberta_spark_{LANGUAGE}",
    learning_rate               = 2e-5,
    ...
    fp16                        = False,   # must be False for DeBERTa-v3
    adam_epsilon                = 1e-6,    # stability
    max_grad_norm               = 1.0,
    report_to                   = "none",
    save_total_limit            = 2,
)
...
# Train & evaluate
# =========================
print(f"\nStarting training — DeBERTa-v3 ({LANGUAGE})...")
trainer.train()
